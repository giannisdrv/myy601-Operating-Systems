...
generic_file_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	ssize_t retval = 0;
	printk(KERN_INFO "Executing now: generic_file_read_iter");
	if (!count)
		return 0; /* skip atime */
...
{
	struct address_space *mapping = file->f_mapping;
	printk(KERN_INFO "Executing now: generic_file_mmap");
...
ssize_t __generic_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	ssize_t		err;
	ssize_t		status;
	printk(KERN_INFO "Executing now: generic_file_write_iter");
