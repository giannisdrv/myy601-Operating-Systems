...
long fat_generic_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	printk(KERN_INFO "Executing now: fat_generic_ioctl");
	switch (cmd) {
...
{
	printk(KERN_INFO "Executing now: fat_file_release");
...
int fat_file_fsync(struct file *filp, loff_t start, loff_t end, int datasync)
{
	...
	int err;
	printk(KERN_INFO "Executing now: fat_file_fsync");
			  ...
			  loff_t offset, loff_t len)
{
	...
	int err = 0;
	printk(KERN_INFO "Executing now: fat_fallocate");
...
static int fat_free(struct inode *inode, int skip)
{
	...
	//start
	length = scnprintf(buffer, sizeof(buffer), "JOURNAL - fat_free: i_start: %d, i_logstart: %d\n", i_start, i_logstart);
	if ( length > 0 ){
		...
		struct kstat *stat, u32 request_mask, unsigned int flags)
{
	...
	struct msdos_sb_info *sbi = MSDOS_SB(inode->i_sb);
	printk(KERN_INFO "Executing now: fat_getattr");
		...
		struct iattr *attr)
{
	...
	int error;
	printk(KERN_INFO "Executing now: fat_setattr");
