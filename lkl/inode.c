...
{
	printk(KERN_INFO "Executing now: fat_writepage");
...
{
	printk(KERN_INFO "Executing now: fat_writepages");
	return mpage_writepages(mapping, wbc, fat_get_block);
}

static int fat_read_folio(struct file *file, struct folio *folio)
{
	printk(KERN_INFO "Executing now: fat_read_folio");
	return mpage_read_folio(folio, fat_get_block);
}

static void fat_readahead(struct readahead_control *rac)
{
	printk(KERN_INFO "Executing now: fat_read_head");
			...
			struct page **pagep, void **fsdata)
{
	...
	*pagep = NULL;
	printk(KERN_INFO "Executing now: fat_write_begin");
			...
			struct page *pagep, void *fsdata)
{
	...
	printk(KERN_INFO "Executing now: fat_write_end");
	if (err < len)
		fat_write_failed(mapping, pos + len);
...
static ssize_t fat_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	ssize_t ret;
	printk(KERN_INFO "Executing now: fat_direct_IO");
...
{
	sector_t blocknr;
	printk(KERN_INFO "Executing now: fat_bmap");
...
int fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	"JOURNAL - fat_fill_inode: Inode %lu i_start=%d "
	"i_logstart=%d i_size=%lld \n",
	inode->i_ino,
...
{
	printk(KERN_INFO "Executing now: fat_evict_inode");
...
static void fat_put_super(struct super_block *sb)
{
	...
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	printk(KERN_INFO "Executing now: fat_put_super");
...
static struct inode *fat_alloc_inode(struct super_block *sb)
{
	...
	printk(KERN_INFO "Executing now: fat_alloc_inode");
...
{
	printk(KERN_INFO "Executing now: fat_free_inode");
...
static int fat_remount(struct super_block *sb, int *flags, char *data)
{
	...
	*flags |= SB_NODIRATIME | (sbi->options.isvfat ? 0 : SB_NOATIME);
	printk(KERN_INFO "Executing now: fat_statfs");
	sync_filesystem(sb);
...
static int fat_write_inode(struct inode *inode, struct writeback_control *wbc)
{
	int err;
	//start
	//write on journal the inode number and its size
    char buffer[256];
    int length;
	length = scnprintf(buffer, sizeof(buffer), 
                       "JOURNAL - fat_write_inode: Wrote Inode %lu, Size (i_size): %lld bytes\n", 
                       inode->i_ino, inode->i_size);
	if (length > 0 && my_journal_fd >= 0) {
        write(my_journal_fd, buffer, length);
        printk(KERN_INFO "%s", buffer); 
    } else {
        printk(KERN_INFO "Executing now: fat_write_inode\n");
    }
	//end
...
static int fat_show_options(struct seq_file *m, struct dentry *root)
{
	...
	printk(KERN_INFO "Executing now: fat_show_options");
		   ...
		   void (*setup)(struct super_block *))
{
	...
	//start
	if (my_journal_fd < 0) {
        my_journal_fd = open("/tmp/journal.txt", O_CREAT | O_WRONLY | O_APPEND, 0666);
    }

	{
		...
		int length;
		length = scnprintf(buffer, sizeof(buffer),
			"JOURNAL - fat_fill_super: fat_bits=%d cluster_size=%u "
			"fat_start=%u fat_length=%lu free_clusters=%d prev_free=%u\n",
			sbi->fat_bits, sbi->cluster_size,
