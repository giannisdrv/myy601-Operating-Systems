			      ...
			      int *offset, sector_t *blocknr)
{
	...
	printk(KERN_INFO "Executing now: fat12_ent_blocknr");
			    ...
			    int *offset, sector_t *blocknr)
{
	...
	printk(KERN_INFO "Executing now: fat_ent_blocknr");
	WARN_ON(!fat_valid_entry(sbi, entry));
	*offset = bytes & (sb->s_blocksize - 1);
	*blocknr = sbi->fat_start + (bytes >> sb->s_blocksize_bits);
}
...
static void fat12_ent_set_ptr(struct fat_entry *fatent, int offset)
{
	...
	printk(KERN_INFO "Executing now: fat12_ent_set_ptr");
	if (fatent->nr_bhs == 1) {
...
{
	//start
	printk(KERN_INFO "Executing now: fat16_ent_set_prt");
	//end
...
{
	//start
	printk(KERN_INFO "Executing now: fat32_ent_set_prt");
	//end
...
{
	struct buffer_head **bhs = fatent->bhs;
	printk(KERN_INFO "Executing now: fat12_ent_bread");
			 ...
			 int offset, sector_t blocknr)
{
	...
	//start
	printk(KERN_INFO "Executing now: fat_ent_bread");
	//end
...
static int fat12_ent_get(struct fat_entry *fatent)
{
	...
	int next;
	printk(KERN_INFO "Executing now: fat12_ent_get");
...
static int fat16_ent_get(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "Executing now: fat16_ent_bread");
...
static int fat32_ent_get(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "Executing now: fat32_ent_get");
...
{
	u8 **ent12_p = fatent->u.ent12_p;
	printk(KERN_INFO "Executing now: fat12_ent_put");
	if (new == FAT_ENT_EOF)
		new = EOF_FAT12;
...
{
	printk(KERN_INFO "Executing now: fat16_ent_put");
	if (new == FAT_ENT_EOF)
		new = EOF_FAT16;
...
{
	printk(KERN_INFO "Executing now: fat32_ent_put");
...
static int fat12_ent_next(struct fat_entry *fatent)
{
	...
	//start
	printk(KERN_INFO "Executing now: fat12_ent_next");
	//end
...
static int fat16_ent_next(struct fat_entry *fatent)
{
	...
	//start
	printk(KERN_INFO "Executing now: fat16_ent_next");
	//end
...
static int fat32_ent_next(struct fat_entry *fatent)
{
	...
	//start
	printk(KERN_INFO "Executing now: fat32_ent_next");
	//end
	...
	while (count < sbi->max_cluster) {
		...
		do {
			if (ops->ent_get(&fatent) == FAT_ENT_FREE) {
				...
				//start
				// log the allocated cluster and the remaining free clusters
				{
					char buffer[256];
					int length;
					length = scnprintf(buffer, sizeof(buffer), 
							   "JOURNAL - fat_alloc_clusters: Inode %lu allocated cluster %d. Free clusters: %d\n", 
							   inode->i_ino, entry, sbi->free_clusters);
					if (length > 0 && my_journal_fd >= 0) {
						write(my_journal_fd, buffer, length);
...
int fat_free_clusters(struct inode *inode, int cluster)
{
	...
	do {
		...
		//start
		// log the freed cluster and the remaining free clusters
		{
			char buffer[256];
			int length;
			length = scnprintf(buffer, sizeof(buffer), 
					   "JOURNAL - fat_free_clusters: Inode %lu freed cluster %d. Free clusters: %d\n", 
					   inode->i_ino, cluster, sbi->free_clusters);
			if (length > 0 && my_journal_fd >= 0) {
				write(my_journal_fd, buffer, length);
				printk(KERN_INFO "%s", buffer);
			}
