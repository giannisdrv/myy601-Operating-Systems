...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    self->active_readers = 0; // set active readers to 0
    self->active_writers = 0; // set active writers to 0
    self->waiting_readers = 0; // set waiting readers to 0
    self->waiting_writers = 0; // set waiting writers to 0
    pthread_mutex_init(&self->lock, NULL); // initialize the mutex lock
    pthread_cond_init(&self->safeToRead, NULL); //initialize the condition safeToRead
    pthread_cond_init(&self->safeToWrite, NULL); //initialize the condition safeToWrite
...
void db_close(DB *self)
{
    ...
    pthread_mutex_destroy(&self->lock); // destroy the mutex lock
    pthread_cond_destroy(&self->safeToRead); // destroy the condition safeToRead
    pthread_cond_destroy(&self->safeToWrite); // destroy the condition safeToWrite
...
{
    pthread_mutex_lock(&self->lock); // acquire the lock for the critical section
    while (self->active_writers > 0 || self->active_readers > 0)
    {
        self->waiting_writers++; // increment waiting writers 
        pthread_cond_wait(&self->safeToWrite, &self->lock); // wait until it's safe to write
        self->waiting_writers--; // decrement waiting writers after being signaled
    }
    
    self->active_writers++; // increment active writers
    pthread_mutex_unlock(&self->lock); // release the lock from exiting the critical section
    ...
    int added = memtable_add(self->memtable, key, value); // add the key-value pair to the memtable

    pthread_mutex_lock(&self->lock); // acquire the lock for the critical section
    self->active_writers--; // decrement active writers after finishing the write operation
    if (self->waiting_writers > 0) // if there are waiting writers, signal one writer to proceed
    {
        pthread_cond_signal(&self->safeToWrite); // signal if any other writers are waiting
    }
    else if (self->waiting_readers > 0) // if there are waiting readers and no waiting writers, signal all readers
    {
        pthread_cond_broadcast(&self->safeToRead); // broadcast all waiting readers if there are any
    }
    pthread_mutex_unlock(&self->lock); // release the lock from exiting the critical section

    return added; // return the result of the add operation
}
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    {
        self->waiting_readers++; // increment waiting readers
        pthread_cond_wait(&self->safeToRead, &self->lock); // wait until it's safe to read
        self->waiting_readers--; // decrement waiting readers after being signaled
    }
    self->active_readers++; // increment active readers
    pthread_mutex_unlock(&self->lock); // release the lock from exiting the critical section
    int got = 0;

    if (memtable_get(self->memtable->list, key, value) == 1)
    {
            got = 1; // if the key is found in the memtable, set got to 1
    }else{
        got = sst_get(self->sst, key, value); // if the key is not found in the memtable, try to get it from the SST files
    }

    pthread_mutex_lock(&self->lock); // acquire the lock for the critical section
    self->active_readers--; // decrement active readers after finishing the read operation
    if (self->waiting_writers > 0 && self->active_readers == 0) // if there are waiting writers and no active readers, signal one writer
    {
        pthread_cond_signal(&self->safeToWrite); // signal if any writers are waiting
    }
    pthread_mutex_unlock(&self->lock); // release the lock from exiting the critical section
    return got; // return the result of the get operation
}
