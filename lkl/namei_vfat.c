				  ...
				  unsigned int flags)
{
	...
	struct dentry *alias;
	int err;
	printk(KERN_INFO "Executing now: vfat_lookup");
	...
	//start
	{
		...
		length = scnprintf(buffer, sizeof(buffer),
			"JOURNAL - vfat_lookup: Found '%.*s' in directory Inode %lu\n", dentry->d_name.len, dentry->d_name.name, dir->i_ino);
		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	struct timespec64 ts;
	int err;
	printk(KERN_INFO "Executing vfat_create");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	...
	//start
	{
        ...
        length = scnprintf(buffer, sizeof(buffer), 
                           "JOURNAL - vfat_create: Created file '%s' in directory Inode %lu\n", 
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "Executing vfat_rmdir");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	...
	//start
	{
        ...
        length = scnprintf(buffer, sizeof(buffer), 
                           "JOURNAL - vfat_rmdir: Removed directory '%s' from Inode %lu\n", 
                           dentry->d_name.name, dir->i_ino);
...
static int vfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "Executing vfat_unlink");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	...
	//start
	{
        ...
        length = scnprintf(buffer, sizeof(buffer), 
                           "JOURNAL - vfat_unlink: Deleted file '%s' from Inode %lu\n", 
                           dentry->d_name.name, dir->i_ino);
		      ...
		      struct dentry *dentry, umode_t mode)
{
	...
	struct timespec64 ts;
	int err, cluster;
	printk(KERN_INFO "Executing vfat_mkdir");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	...
	//start
	{
        ...
        length = scnprintf(buffer, sizeof(buffer), 
                           "JOURNAL - vfat_mkdir: Created directory '%s' in Inode %lu\n", 
                           dentry->d_name.name, dir->i_ino);
		       ...
		       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	printk(KERN_INFO "Executing now: vfat_rename");
