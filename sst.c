...
static void merge_thread(void* data)
{
    ...
    while (1)
    {
...
#else
        {
            ...
            if (rt == ETIMEDOUT)
            {
                if (sst->num_files[0] >= MAX_FILES_LEVEL0)
                {
                    ...
                    pthread_mutex_unlock(&sst->cv_lock); // Unlock before evaluating compaction and performing compaction to allow other threads to proceed with their work while we are doing the compaction
                    _evaluate_compaction(sst);
                    sst_compact(sst);

                    pthread_mutex_lock(&sst->cv_lock); // Lock again after compaction is done to check the merge state and continue with the next iteration of the loop
                }
                goto exit;
            }
        }
            ...
            sst->immutable)
        {
            ...
            pthread_mutex_unlock(&sst->cv_lock);// Unlock before merging to allow other threads to proceed with their work while we are doing the merge
            sst_merge_real(sst, sst->immutable_list);
            pthread_mutex_lock(&sst->cv_lock); // Lock again after merge is done to check the merge state and continue with the next iteration of the loop
