...
int fat_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
{
	...
	//start
	//log how many entries were removed from which directory
	{
		char buffer[256];
		int length;
		length = scnprintf(buffer, sizeof(buffer), 
				   "JOURNAL - fat_remove_entries: Removed %d entries from directory Inode %lu\n", 
				   sinfo->nr_slots, dir->i_ino);
...
found:
	...
	//start
	//log the slots that were requested
	{
        char buffer[256];
        int length;
        length = scnprintf(buffer, sizeof(buffer), 
                           "JOURNAL - fat_add_entries: Added %d entries in directory Inode %lu\n", 
                           sinfo->nr_slots, dir->i_ino);
